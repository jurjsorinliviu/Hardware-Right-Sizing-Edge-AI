# Hardware Right-Sizing for Sustainable Edge AI: Extracting Deployment Requirements from Physics Structure-Informed Neural Networks Under Renewable Intermittency

[![License](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](https://opensource.org/licenses/Apache-2.0)
[![Python 3.8+](https://img.shields.io/badge/python-3.8+-blue.svg)](https://www.python.org/downloads/)
[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://codespaces.new/jurjsorinliviu/Hardware-Right-Sizing-Edge-AI)
...
1. **Hardware over-specification** — extracts quantitative deployment requirements (operations, memory, power) directly from the Psi-NN's discovered structure, enabling right-sized platform selection before any hardware is committed.
2. **Carbon footprint** — the platform change that structure-extraction enables (Jetson Orin Nano → STM32H7) yields ~18× lifecycle carbon reduction per device; solar-constrained training contributes under 1% of this.
3. **Renewable-power feasibility** — characterizes what intermittent training actually does via an orthogonal five-cell decomposition (n=10 seeds, paired bootstrap, 95% CI) across seven PDE problems, isolating regularization, budget, and schedule contributions.

### Key Results

| Metric                  | Value                                                                                |
| ----------------------- | ------------------------------------------------------------------------------------ |
| **Cost Savings**        | 96.8% ($249 Jetson Orin Nano → $8 STM32H7)                                          |
| **Carbon Reduction**    | ~18× per device (238 kg → ~13.3 kg CO₂ over 5-year lifecycle)                       |
...
| **Budget sensitivity**  | C→B spans three orders of magnitude across seven PDEs (5 of 7 resolved at 95% CI)    |

## 🎯 Key Contributions

### 1. Hardware Requirement Extraction from Discovered Structure

Reads quantitative, a priori hardware requirements directly from the parameter-tying, symmetry, and sparsity patterns the Psi-NN discovers:
...
The dominant factor in training outcome under solar constraints is **budget sensitivity** — the C→B contrast at matched regularization. It is resolved on five of seven test problems and spans three orders of magnitude (Burgers −3.2% to Advection +620.0%). **No clean predictor emerged across the PDE classes and structural properties tested at n = 7** — this is suggestive but not strong evidence of nonexistence; broader-coverage studies would be needed before this absence could be claimed strongly. Practitioners must therefore measure C→B per problem before committing to renewable-powered training.
...
- All contrasts D-normalized for Table III (additive closure residual ≤ 4×10⁻¹⁴); C-normalized for Table IV (budget sensitivity)
...
├── generate_figure2_timeline.py    # Figure 2 (three-regime timeline)
├── generate_figure3_decomposition.py  # Figure 3 (five-cell decomposition)
├── generate_figure4_kappa_sweep.py # Figure 4 (Burgers κ-sweep curve)
│
...
│   ├── kappa_sweep_experiment.py         # Figure 4 (κ-sweep on Burgers)
│   ├── duty_cycle_sweep.py
...
│   ├── export_results.py
│   ├── methodology_pipeline.html         # Figure 1 source (primary HTML)
│   ├── methodology_pipeline vertical.html
│   ├── methodology_pipeline_horizontal.html
│   └── methodology_pipeline_vertical_less_color.html
│
...
└── results/                              # Experimental outputs (CPU backend, blessed)
    ...
    ├── control_arm/                       # D, C, B, A cells for Table III decomposition
    ├── burgers_kappa_sweep/               # κ ∈ {0, 0.5, 1.0, 1.5, 2.0} for Figure 4
    ├── kappa_sweep_burgers/               # Legacy κ-sweep (pre-revision reference)
...
```bash
git clone https://github.com/jurjsorinliviu/Hardware-Right-Sizing-Edge-AI.git
cd Hardware-Right-Sizing-Edge-AI
...
```bash
# Figure 2: Three-regime training timeline (24-hour cycle)
python generate_figure2_timeline.py

# Figure 3: Five-cell orthogonal decomposition framework
python generate_figure3_decomposition.py

# Figure 4: Burgers PDE κ-sweep improvement curve
python generate_figure4_kappa_sweep.py
```

Figure 1 (methodology pipeline) is built from the HTML source at
`experiments/methodology_pipeline.html` and rendered to PNG/JPG via a headless
...
**Key Finding**: C→B spans three orders of magnitude. **No clean predictor emerged
across the PDE classes and structural properties tested at n = 7** (suggestive but
not strong evidence of nonexistence) — parabolic Heat (+176.8%) ranks above
transport-dominated Advection in some C-normalized framings depending on baseline
choice; Laplace and Burgers stay low-sensitivity. The Memristor's sign reversal
...
### Figure 4 — Burgers PDE κ-Sweep (Weak-Monotone Improvement)
...
| --------------------- | -------- | ---------- | ----------- | ----------- |
| **Solar + STM32H7**   | 0.036 kg | 13.3 kg    | **~13.3 kg** | **~18× less** |
| Grid + Jetson Orin    | 0.356 kg | 238 kg     | **238 kg**  | baseline    |

**Per-device saving: 224.7 kg CO₂. ~99.9% of this is from the hardware change
(Jetson → STM32H7); solar-constrained training contributes <1% (~0.32 kg).**

At scale (10,000 devices): ~2,247 metric tons CO₂ saved.
At scale (1M devices): ~224,700 metric tons CO₂ saved.
...
- **Table III**: D-normalized (additive closure D→C + C→B + B→E = D→E, residual ≤ 4×10⁻¹⁴)
- **Table IV**: C-normalized (budget sensitivity relative to full-budget at high reg)
- **κ-sweep (Figure 4)**: D-normalized (improvement relative to continuous baseline)
...
| Decomposition (Table III) | 10 × 4 cells × 7 | 3000 | ~80 hours        |
| κ-sweep (Figure 4)     | 10 × 5      | 3000    | ~14 hours           |
...
@article{jurj2026right_sizing,
  author  = {Sorin Liviu Jurj},
  title   = {Hardware Right-Sizing for Sustainable Edge AI:
             Extracting Deployment Requirements from
             Physics Structure-Informed Neural Networks
             Under Renewable Intermittency},
  journal = {Under Review},
  year    = {2026},
  url     = {https://github.com/jurjsorinliviu/Hardware-Right-Sizing-Edge-AI}
}
```
...
1. **Hardware right-sizing is the dominant sustainability lever** — of the 224.7 kg
   per-device carbon saving, ~99.9% comes from the Jetson → STM32H7 platform
   change enabled by structure-extraction; <1% comes from solar-constrained
...
4. **Budget sensitivity (C→B) spans three orders of magnitude.**
...
- **Reduction**: ~224.7 kg CO₂ per device (≈ 94.4% less, or ~18× less)
...
| ---------------- | ------------ | ------------- | ------------ | ------------------------------- |
| 1,000 devices    | 238 tons     | 13.3 tons     | 224.7 tons   | ~49 cars removed for 1 year     |
| 10,000 devices   | 2,380 tons   | 133 tons      | 2,247 tons   | ~489 cars / ~2,675 acres forest |
| 1M devices (IoT) | 238,000 tons | 13,300 tons   | 224,700 tons | ~48,800 cars / ~267,000 acres   |
